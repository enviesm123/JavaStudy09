...
import ch10_extends_interface.simple.Child;
...
public class StarcraftMain {
    public static void main(String[] args) {
        ...
        // ArrayList<Zealot> 에 HighTamplar를 담을 수 없다.
//        zealotList.add(highTemplar);
        ...
        starList.add(highTemplar); // HighTemplar가 StarUnit으로 자동 형변환 되며 담김

        for(int i = 0; i < starList.size(); i++){
            starList.get(i).move(200, 150);
        }
...
//        starList.get(3).psionincStorm();

        // HighTemplar -> StarUnit 을 다시
        // StarUnit -> HighTemplar로 되돌려 준 후 메소드 사용 가능
        ...
        for(int i = 0; i < starList.size(); i++){
            if(starList.get(i).equals("하이템플러")){
